...
- **Breadcrumb, decor icon, and the YouTube embed/link moved OUTSIDE the post card entirely**, into a new plain `.post-meta-top` strip that sits above the card, on the ordinary page background — no border, no per-section tint. They now always use the site's one fixed teal brand color (`#009F93` / `#4FD1C5` in dark mode), the same on every post regardless of section, instead of the section's `--accent`. Only the title + divider + body copy remain inside the tinted `.post-body` card. If you're adding a new field to the header, decide up front whether it belongs in this plain outer strip (site chrome) or inside the card (part of the post's own "paper") — don't just drop it into `.post-header` without thinking about which side of that split it belongs on.
